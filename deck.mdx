...
<div style={{ fontSize: 60, textAlign: "left" }}>
  <ul>
      ...
      <li>What is difference between TDD and BDD?</li>
      <li>What are the different types of tests?</li>
      <li>What tools do we use to write tests?</li>      
  </ul>
</div>
...
<div style={{ fontSize: 100 }}>
  <div>TDD versus BDD.</div>
</div>

<Notes>
  TDD is a software development process that relies on the repetition of a very
  short development cycle. BDD also writes tests before code, but the focus of
  BDD is to simulate what the user would do in a prod environment? OPINION: When
  your boss hands you a big project... say adding an additional page to your app
  with a lot of logic that your going to have to create. My thought is to create
  a unit test for one specific function you know you will need to complete the
  task(that fails) create the function to pass the test and then create the next
  unit test etc etc...
</Notes>
...
<div style={{ fontSize: 100 }}>
  <div>What are the different types of tests?</div>
  <ul>
    <div style={{ fontSize: 60, textAlign: "left" }}>
      <Appear>
        <li>Unit Testing</li>
...
<Notes>
  TODO: An Integration test is determining if independently developed units or
  component work correctly when connected. An End to End test would be a series
  of tests on a system. Think about logging into your banking website than
  checking the balance and than logging out. An end to end test would check that
  you logged in correctly clicked a button to check your balance and than checks
  to make sure you logged out correctly. Automated testing is usually browser
  based and it would simulate the behavior that would be expected from a real
  user.
</Notes>
...
<CodeSurfer 
  ...
  theme={dracula}
    ...
    { range: [1, 4], notes: "Takes in two variables, adds them together and returns an answer. Done!" },  
    ...
    { range: [23, 26], notes: "It passed, but I might to need to talk to my boss about how many decimal places are acceptable." },
...
<CodeSurfer 
  ...
  theme={dracula}
    steps={[
    { notes: ""},
    { range: [1, 6], notes: "This is what code you find in the wild looks like." },  
    { range: [9, 16], notes: "I added additional code and these tests are still passing. Confidence++" },
    { range: [17, 20], notes: "That's what's suppose to happen when you pass 2 strings." },
    { range: [21, 24], notes: "Probably not going to happen, but atleast when someone asks you about passing boolean values you can answer confidently." },
    { range: [25, 28], notes: "Confidence++" },
    { range: [29, 32], notes: "I can even handle stupid use cases that probably won't happen." },
  ]}
  />;
...
<Notes>
  ...
  4) The more tests you write the better your tests will be and the more bugs you will catch before they get to QA or worse PROD!
  5) TODO: What else????
</Notes>

---

<div style={{ fontSize: 100 }}>Does Anyone Have Any Questions?</div>
...
<div style={{ fontSize: 60, textAlign: "left" }}>
  <ul>
      ...
      <li>Vaco for out meetup.com membership</li>
      <li>Adabra for providing the food</li>
      <li>My(former) co-workers at CJC for helping with the content for this
